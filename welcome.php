...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if(empty(trim($_POST["bookingDate"]))){
		...
		$sql = "SELECT id FROM vehicle_booking  where bookingDate = ? group by id Having Count(bookingType)>=16 ;";
		$sql1 = "SELECT id FROM vehicle_booking  where DAYOFWEEK(?)= 1;";
                ...
                if(mysqli_stmt_num_rows($stmt) > 1){
                    $bookingDate_err = "Today reservations are filled";
						...
						if($stmt2 = mysqli_prepare($link, $sql1)){
							// Bind variables to the prepared statement as parameters
							mysqli_stmt_bind_param($stmt2, "s", $param_bookingDate);
							...
							if(mysqli_stmt_execute($stmt2)){
								...
								if(mysqli_stmt_num_rows($stmt2) > 0){
									$bookingDate_err = "Sorry, We are Closed on Sunday";
								} else{
    ...
    if(empty($vehicleType_err) && empty($vehicleLN_err) && empty($vehicleMake_err) && empty($engineType_err) && empty($bookingType_err) && empty($bookingDate_err) ){
		...
		$sql2 = "INSERT INTO reservation (user, vehicleLN) VALUES (?, ?)";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
			...
			$param_anynote = $anynote;
				...
				if($stmt2 = mysqli_prepare($link, $sql2)) {
					mysqli_stmt_bind_param($stmt2, "ss",$_SESSION["username"] , $param_vehicleLN);
					$param_vehicleLN = $vehicleLN;
					
					if(mysqli_stmt_execute($stmt2)){
						// Redirect to booking reserved page
...
<body class="main-layout">
    ...
    <p style="text-align: center;">
	   <a href="book.php" class="btn btn-danger">Previuos Bookings</a>
    </p>
